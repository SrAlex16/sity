...
def test_gmail_search_not_connected_returns_clear_message() -> None:
    ...
    with patch("app.tools.handlers.google_tools._resolve_google_creds", return_value=None):
...
def test_calendar_create_event_requires_title_start_end() -> None:
    ...
    with patch("app.tools.handlers.google_tools._resolve_google_creds", return_value=MagicMock()):
        result = handle_calendar_create_event(ctx)
    assert result.ok is False
